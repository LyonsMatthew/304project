    ...
    (lambda (lexp bounded)
        (if (expression? lexp)
            (cases expression lexp
                [var-exp (id) 
                    (let ([lexical-coord (find-lexical-coord id bounded)])
                        (if (not lexical-coord)
                            ...
                            (if (is-refed-lexically id bounded)
                                (ref-bound-exp (car lexical-coord) (cadr lexical-coord))
                                (var-exp-but-it-is-actually-not-a-var-exp-and-also-it-is-bound (car lexical-coord) (cadr lexical-coord)))))]
                [lambda-exp (ids body)
                    (let* ([new_ids (map (lambda (x) (if (expression? x) (lexical-address-helper x bounded) x)) ids)]
                            [new_bounded (append (list new_ids) bounded)])
                            (newline)
                            (display new_ids)
                            (newline)
                            (display "------------")
                            ...
                            (display "=============")
                            (newline)
                        (lambda-exp new_ids (in-order-map 
                ...
                [ref-exp (var)
                    (let ([lexical-coord (find-lexical-coord var bounded)])
                        (if (not lexical-coord)
                            (var-exp-but-it-is-actually-not-a-var-exp-and-also-it-is-free var)
                            (if (is-refed-lexically var bounded)
                                (ref-bound-exp (car lexical-coord) (cadr lexical-coord))
                                (var-exp-but-it-is-actually-not-a-var-exp-and-also-it-is-bound (car lexical-coord) (cadr lexical-coord)))))]
                [else "error"])
...
(define lookup-bound-by-lexical-address
	(lambda (old-env depth position)
		(cases environment old-env
			[extended-env-record (syms vals env) 
				(if (eqv? depth 0)
					(deref (lookup-bound-by-lexical-position vals position))
					(lookup-bound-by-lexical-address env (- depth 1) position))]
			[else 876543456787654])))
            
(define lookup-ref-by-lexical-address
	(lambda (old-env depth position)
		(cases environment old-env
			[extended-env-record (syms vals env) 
				(if (eqv? depth 0)
					(lookup-bound-by-lexical-position syms position)
					(lookup-bound-by-lexical-address env (- depth 1) position))]
			[else 876543456787654])))
					...
					[var-exp (var) 
						(if (expression? (car syms))
							(cases expression (car syms) ; always a ref-exp
								[ref-bound-exp (depth position) (cons (easy-apply-env-ref env (lookup-ref-by-lexical-address env depth position)) (convert-args (cdr syms) (cdr args) env))]
								[else 765456787])
		...
		(if (null? syms)
			...
			(if (expression? (car syms))
				(cases expression (car syms)
					[ref-bound-exp (depth position) (cons (lookup-ref-by-lexical-address env depth position) (convert-refs-to-syms (cdr syms)))]
					[else 763476543])
